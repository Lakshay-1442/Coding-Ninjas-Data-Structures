...
pair<int, int> getMinAndMax(BinaryTreeNode<int> *root) 
{
    ...
    int lmax = smallAns1.second;
    int rmax = smallAns2.second;
    
    int lmin = smallAns1.first;
    int rmin = smallAns2.first;
    
    int min_result = min(root -> data, min(lmin, rmin));
    int max_result = max(root -> data, max(lmax, rmax));
    
    pair<int, int> result;
    
    result.first = min_result;
    result.second = max_result;
