...
bool isPossible(int **maze, int **soln, int n, int row, int col)
{
    ...
    if (maze[row][col] == 0 || soln[row][col] == 1)
    {
        return false;
    }
...
void Rat_in_a_Maze(int **maze, int **soln, int n, int row, int col)
{
    // Base Case
    if (row == n - 1 && col == n - 1)
    {
    ...
    if (isPossible(maze, soln, n, row, col))
    {
        soln[row][col] = 1;
        
        Rat_in_a_Maze(maze, soln, n, row - 1, col);
        Rat_in_a_Maze(maze, soln, n, row + 1, col);
        Rat_in_a_Maze(maze, soln, n, row, col - 1);
        Rat_in_a_Maze(maze, soln, n, row, col + 1);
